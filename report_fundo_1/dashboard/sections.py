"""Report Fundo 1 sections built from the original R report logic."""
...
def _plain_axis(series: pd.Series) -> list:
    """Plain Python values for a Plotly axis.

    plotly.py 6 encodes numpy/pandas arrays as base64 typed arrays, which the
    plotly.js bundled with older Streamlit builds (the Snowflake runtime) can't
    decode: it silently plots the row index instead of the real values. Only
    values handed to the figure as plain lists survive that round trip.
    """
...
def _line_chart(df: pd.DataFrame, x_col: str, y_col: str, color: str) -> go.Figure:
    fig = go.Figure()
    fig.add_scatter(
        x=_plain_axis(df[x_col]),
        y=_plain_axis(df[y_col]),
        mode="lines+markers",
        name=y_col,
...
def _multi_line_chart(df: pd.DataFrame, x_col: str, y_cols: list[str]) -> go.Figure:
    fig = go.Figure()
    x_values = _plain_axis(df[x_col])
    for index, y_col in enumerate(y_cols):
        fig.add_scatter(
            ...
            mode="lines+markers",
            name=y_col,
            line=dict(color=PLOTLY_COLORWAY[index % len(PLOTLY_COLORWAY)]),
        )
    return fig
...
def _safe_ratio(numerator: pd.Series, denominator: pd.Series) -> pd.Series:
    """Vectorized _safe_div: zero or missing denominator yields NaN."""
    num = pd.to_numeric(numerator, errors="coerce")
    den = pd.to_numeric(denominator, errors="coerce")
    return (num / den.where(den != 0)).replace([np.inf, -np.inf], np.nan)


def _weighted_parts(values: pd.Series, weights: pd.Series) -> tuple[pd.Series, pd.Series]:
    """Numerator and denominator of a weighted average, summable with groupby.agg.

    Rows the scalar _weighted_average would discard contribute zero to both.
    """
    val = pd.to_numeric(values, errors="coerce").replace([np.inf, -np.inf], np.nan)
    wgt = pd.to_numeric(weights, errors="coerce").replace([np.inf, -np.inf], np.nan)
    valid = val.notna() & wgt.notna() & (wgt > 0)
    return (val * wgt).where(valid, 0.0), wgt.where(valid, 0.0)
...
def _perf_by_period(
    ...
    old enough for a title to be classifiable at the report snapshot.
    """
...
def _perf_by_rolling_window(df: pd.DataFrame, ref_date: date, pd_days: int, window_days: int, pe_days: int = 90) -> pd.DataFrame:
    ...
    daily = (
        data.assign(
            _ead_pd=data["valor_parcela"] * data["is_pd"],
            _ead_pd_unp=data["valor_parcela"] * data["is_pd"] * data["unpaid"],
        )
        .groupby("due_dt", as_index=False)
        .agg(
            gmv=("valor_parcela", "sum"),
            ead_pd=("_ead_pd", "sum"),
            ead_pd_unp=("_ead_pd_unp", "sum"),
        )
    )
    ...
    daily["pd"] = _safe_ratio(daily["ead_pd_r"], daily["gmv_r"])
    daily["lgd_raw"] = _safe_ratio(daily["ead_pd_unp_r"], daily["ead_pd_r"])
...
def _plot_metric_lines(df: pd.DataFrame, x_col: str, y_cols: list[str], title: str, y_title: str = "%") -> go.Figure:
    fig = _multi_line_chart(df, x_col, y_cols)
...
def render_visao_geral() -> None:
    ...
    # Listas puras em vez de Series: o plotly do Snowflake descarta os arrays
    # quando recebe Series direto em add_bar/add_scatter, e o grafico deita.
    months = monthly["year_month"].dt.strftime("%Y-%m-%d").tolist()
    amounts = [float(v) for v in monthly["loan_amount"]]
    cum_amounts = [float(v) for v in monthly["cum_amount"]]
    fig = go.Figure()
    fig.add_bar(x=months, y=amounts, orientation="v", name="Valor originado no mês (EAD)", marker_color=COLORS["primary"])
    fig.add_scatter(x=months, y=cum_amounts, name="Valor acumulado originado", mode="lines+markers", yaxis="y2", line=dict(color=COLORS["secondary"]))
    ...
    taxa_num, taxa_den = _weighted_parts(df["taxa"], df["valor_parcela"])
    prazo_num, prazo_den = _weighted_parts(df["prazo"], df["valor_parcela"])
    pricing = (
        df.assign(_taxa_num=taxa_num, _taxa_den=taxa_den, _prazo_num=prazo_num, _prazo_den=prazo_den)
        .groupby("year_month", as_index=False)
        .agg(
            ticket_medio=("valor_parcela", "mean"),
            _taxa_num=("_taxa_num", "sum"),
            _taxa_den=("_taxa_den", "sum"),
            _prazo_num=("_prazo_num", "sum"),
            _prazo_den=("_prazo_den", "sum"),
        )
    )
    pricing["taxa_media"] = _safe_ratio(pricing["_taxa_num"], pricing["_taxa_den"])
    pricing["prazo_medio"] = _safe_ratio(pricing["_prazo_num"], pricing["_prazo_den"])
    pricing = pricing[["year_month", "taxa_media", "prazo_medio", "ticket_medio"]]
    c1, c2 = st.columns(2)
    with c1:
        fig_rate = _line_chart(pricing, "year_month", "taxa_media", COLORS["secondary"])
        fig_rate.update_layout(**get_standard_layout(title="Taxa média ponderada (% a.m.)", show_legend=False, margin=dict(l=40, r=16, t=64, b=40)))
        _render_chart(fig_rate)
    with c2:
        fig_tenor = _line_chart(pricing, "year_month", "prazo_medio", COLORS["accent"])
        fig_tenor.update_layout(**get_standard_layout(title="Prazo médio ponderado (dias)", show_legend=False, margin=dict(l=40, r=16, t=64, b=40)))
        _render_chart(fig_tenor)
...
def render_carteira() -> None:
    ...
    if conc.empty:
        ...
        fig = go.Figure()
        fig.add_bar(
            x=_plain_axis(conc_plot["total"]),
            y=_plain_axis(conc_plot["seller"]),
            orientation="h",
            text=_plain_axis(conc_plot["share_label"]),
    ...
    prazo_num, prazo_den = _weighted_parts(df["prazo"], df["valor_parcela"])
    taxa_num, taxa_den = _weighted_parts(df["taxa"], df["valor_parcela"])
    seller_base = df.assign(
        _prazo_num=prazo_num,
        _prazo_den=prazo_den,
        _taxa_num=taxa_num,
        _taxa_den=taxa_den,
        _on_book_face=df["valor_parcela"].where(df["on_book"] == 1, 0.0),
        _stage2=df["valor_parcela"].where(df["stage"] == "Stage 2", 0.0),
        _stage3=df["valor_parcela"].where(df["stage"] == "Stage 3", 0.0),
        _late30=df["valor_parcela"].where(df["days_late"] > 30, 0.0),
        _late90=df["valor_parcela"].where(df["days_late"] > 90, 0.0),
    )
    seller_metrics = seller_base.groupby("seller", as_index=False).agg(
        gmv_total=("valor_parcela", "sum"),
        on_book_face=("_on_book_face", "sum"),
        n_operacoes=("chave_nfe", "nunique"),
        n_pme=("sacado_nome", "nunique"),
    ...
    seller_metrics["prazo_medio_wt"] = _safe_ratio(seller_metrics["_prazo_num"], seller_metrics["_prazo_den"])
    seller_metrics["taxa_media_wt"] = _safe_ratio(seller_metrics["_taxa_num"], seller_metrics["_taxa_den"])
    seller_metrics["stage2_share"] = _safe_ratio(seller_metrics["_stage2"], seller_metrics["_late30"])
    seller_metrics["stage3_share"] = _safe_ratio(seller_metrics["_stage3"], seller_metrics["_late90"])
    ...
    c1, c2 = st.columns(2)
    with c1:
        fig_sme = _line_chart(df_plot, "year_month", "accum_companies", COLORS["secondary"])
        fig_sme.update_layout(**get_standard_layout(title="PMEs acumuladas ao longo do tempo", show_legend=False, margin=dict(l=40, r=16, t=64, b=40)))
        _render_chart(fig_sme)
    with c2:
        fig_des = _line_chart(df_plot, "year_month", "desagio", COLORS["accent"])
        fig_des.update_layout(**get_standard_layout(title="Receita de deságio (mensal)", show_legend=False, margin=dict(l=40, r=16, t=64, b=40)))
        _render_chart(fig_des)
    collect_columns_start(2)
...
def _render_seller_drilldown(
    ...
    *,
    ...
    taxa_num, taxa_den = _weighted_parts(seller_df["taxa"], seller_df["valor_parcela"])
    orig = (
        seller_df.assign(_taxa_num=taxa_num, _taxa_den=taxa_den)
        .groupby("year_month", as_index=False)
        .agg(
            origination=("valor_parcela", "sum"),
            _taxa_num=("_taxa_num", "sum"),
            _taxa_den=("_taxa_den", "sum"),
        )
    )
    orig["taxa_media"] = _safe_ratio(orig["_taxa_num"], orig["_taxa_den"])
    orig = orig[["year_month", "origination", "taxa_media"]]
    ...
    if risk.empty:
        ...
        elig30 = risk["eligible30"] == 1
        lgd_scope = (risk["is30"] == 1) & (risk["eligible_lgd"] == 1)
        risk_month = (
            risk.assign(
                _base_30=risk["valor_parcela"].where(elig30, 0.0),
                _ead_30=(risk["valor_parcela"] * risk["is30"]).where(elig30, 0.0),
                _ead_30_unpaid_proxy=(risk["valor_parcela"] * risk["is30"] * risk["unpaid"]).where(elig30, 0.0),
                _lgd_base=risk["valor_parcela"].where(lgd_scope, 0.0),
                _ead_30_unpaid=(risk["valor_parcela"] * risk["unpaid"]).where(lgd_scope, 0.0),
            )
            .groupby("year_month_expire", as_index=False)
            .agg(
                base_30=("_base_30", "sum"),
                ead_30=("_ead_30", "sum"),
                ead_30_unpaid_proxy=("_ead_30_unpaid_proxy", "sum"),
                lgd_base=("_lgd_base", "sum"),
                ead_30_unpaid=("_ead_30_unpaid", "sum"),
            )
        )
        risk_month["pd30"] = _safe_ratio(risk_month["ead_30"], risk_month["base_30"])
        risk_month["lgd"] = _safe_ratio(risk_month["ead_30_unpaid"], risk_month["lgd_base"])
        risk_month["pe_proxy"] = _safe_ratio(risk_month["ead_30_unpaid_proxy"], risk_month["base_30"])
    ...
    fig_book = go.Figure()
    fig_book.add_bar(
        x=_plain_axis(book_status["bucket"]),
        y=_plain_axis(book_status["amount"]),
        orientation="v",
    ...
    fig_orig = go.Figure()
    fig_orig.add_bar(
        x=_plain_axis(orig["year_month"]),
        y=_plain_axis(orig["origination"]),
        orientation="v",
    ...
    fig_rate = _line_chart(orig, "year_month", "taxa_media", COLORS["accent"])
    ...
    fig_risk = _multi_line_chart(risk_month, "year_month_expire", ["pd30", "lgd", "pe_proxy"])
    ...
    if render_ui:
        c1, c2 = st.columns(2)
        with c1:
            _render_chart(fig_book)
        with c2:
            _render_chart(fig_orig)
        c3, c4 = st.columns(2)
        with c3:
            _render_chart(fig_rate)
        with c4:
            _render_chart(fig_risk)
