...
def _load_private_key(key_path: str) -> bytes:
    """Load a PEM/DER private key file for Snowflake auth."""
...
def _get_snowflake_connection():
    ...
    pk_path = conn_params.pop("private_key_file", None)
    if pk_path:
        conn_params["private_key"] = _load_private_key(pk_path)

    required_params = ["user", "account"]
    has_auth = "password" in conn_params or "private_key" in conn_params
    missing = [p for p in required_params if p not in conn_params or not conn_params[p]]
    if missing or not has_auth:
        raise ValueError(
            f"Missing required Snowflake config: {missing or ['password or private_key_file']}. "
            "Check your .env file."
        )
