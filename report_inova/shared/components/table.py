...
def render_table_with_merged_headers(
    ...
    expand_to_container: bool = True,
    max_body_height: int | None = None,
    container_align: str = "center",
) -> None:
    ...
    table_class = "merged-header table-expanded" if expand_to_container else "merged-header table-compact"
    html = f'''
    ...
    table.merged-header.table-expanded {{
        width: 100%;
        table-layout: fixed;
    }}
    table.merged-header.table-compact {{
        width: auto;
        max-width: 100%;
        table-layout: auto;
    }}
    table.merged-header.table-compact th,
    table.merged-header.table-compact td {{
        white-space: normal;
    ...
    if column_width_overrides:
        ...
        html += "</style>"
    # Optional container width/position/scroll styling
    container_style_rules = []
    if container_css_width:
        container_style_rules.append(f"width: {container_css_width}")
    elif expand_to_container:
        container_style_rules.append("width: 100%")
    else:
        container_style_rules.append("width: fit-content")
        container_style_rules.append("max-width: 100%")

    if max_body_height is not None:
        container_style_rules.append(f"max-height: {int(max_body_height)}px")
        container_style_rules.append("overflow-y: auto")
        container_style_rules.append("overflow-x: hidden")

    align = (container_align or "left").lower()
    if align == "center":
        container_style_rules.append("margin-left: auto")
        container_style_rules.append("margin-right: auto")
    elif align == "right":
        container_style_rules.append("margin-left: auto")
        container_style_rules.append("margin-right: 0")

    container_style_attr = f' style="{"; ".join(container_style_rules)};"' if container_style_rules else ""
    ...
    if num_levels == 3:
        ...
        html += f'<table class="{table_class}" id="{table_id}"><thead><tr><th rowspan="{num_levels}" style="text-align: center;" title="Forma de Pagamento">Forma de Pagamento</th>'
        ...
        html += f'<table class="{table_class}" id="{table_id}"><thead><tr><th rowspan="2" style="text-align: center;" title="Forma de Pagamento">Forma de Pagamento</th>'
        ...
        html += f'<table class="{table_class}" id="{table_id}"><thead><tr>'
